...
show_film = CallbackData("show_film", "film_id")


def get_better_pages_keyboard_films(sliced_array, page: int = 1):
    key = "films"
    ...
    for text in sliced_array:
        ...
        texts_buttons.append(
            InlineKeyboardButton(
                ...
                callback_data=show_film.new(film_id=text[0])
            )
        )
    ...
    count_rows_in_db = db.count_number_of_rows_in_table("films")
    ...
    markup.row(add_item_keyboard(item_category="films")[1])
    return markup
